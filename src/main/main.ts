...
app.setName("NahkriinOS");
app.setAppUserModelId("com.nahkriin.os");
...
async function createOverlayWindow() {
  ...
  overlayWindow = new BrowserWindow({
    ...
    hasShadow: false,
    title: "NahkriinOS Performance Overlay",
    webPreferences: {
      preload: path.join(__dirname, "../preload/preload.cjs"),
      contextIsolation: true,
      nodeIntegration: false,
...
public class NahkriinWindowApi {
...
"@
$hwnd = [NahkriinWindowApi]::GetForegroundWindow()
$rect = New-Object NahkriinWindowApi+RECT
[void][NahkriinWindowApi]::GetWindowRect($hwnd, [ref]$rect)
$pidValue = 0
[void][NahkriinWindowApi]::GetWindowThreadProcessId($hwnd, [ref]$pidValue)
...
async function createDimWindow(display: Electron.Display, settings: AppSettings["entertainment"]) {
  ...
  const win = new BrowserWindow({
    ...
    backgroundColor: "#00000000",
    hasShadow: false,
    title: "NahkriinOS Watching Mode Dimmer",
...
function createTray() {
  try {
    ...
    tray.setContextMenu(Menu.buildFromTemplate([
      { label: "Show NahkriinOS", click: () => { mainWindow?.show(); mainWindow?.focus(); } },
...
function showReminderNotification(reminder: ReminderItem) {
  ...
  const notification = new Notification({
    title: reminder.title || reminder.text || "NahkriinOS reminder",
...
function wireIpc() {
  ...
  ipcMain.handle("reminders:testNotification", () => {
    showReminderNotification({ id: "test", text: "Notifications are working.", title: "NahkriinOS reminder test", notes: "You will see local reminders here when they are due.", dueAt: new Date().toISOString(), completed: false, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
  ...
  ipcMain.handle("system:stressExport", async () => {
    const data = await store.read();
    const file = path.join(app.getPath("documents"), `NahkriinOS-Stress-History-${Date.now()}.json`);
