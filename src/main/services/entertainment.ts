...
export async function generateEntertainmentRecommendations(store: JsonStore): Promise<EntertainmentRecommendation[]> {
  ...
  const response = await fetch(RESPONSES_URL, {
    ...
    body: JSON.stringify({
      ...
      instructions: "You are NahkriinOS entertainment assistant. Recommend games/shows/movies in a consumer-friendly way from local play/watch history. Return only JSON array. Categories: Tonight's Pick, Continue Playing, Continue Watching, Short Session, Hidden Gem, Insight. Do not invent exact external metadata if not provided.",
