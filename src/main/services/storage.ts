...
const defaultSettings: AppSettings = {
  ...
  discord: {
    ...
    backendUrl: process.env.NAHKRIINOS_REMINDER_BACKEND_URL || "",
    syncEnabled: true
  },
  updates: {
    enabled: true,
...
export class JsonStore {
  private filePath = path.join(app.getPath("userData"), "nahkriinos-data.json");
  ...
  async clearApiKey(): Promise<void> {
    await fs.rm(path.join(app.getPath("userData"), "nahkriinos-secrets.bin"), { force: true });
  ...
  async getDiscordBackendToken(): Promise<string> {
    if (process.env.NAHKRIINOS_REMINDER_BACKEND_TOKEN?.trim()) return process.env.NAHKRIINOS_REMINDER_BACKEND_TOKEN.trim();
    try {
